...
def run_energy_analysis(logits_train, logits_test, logits_ood, temp=1.0):
    print(f"\n>>> 开始执行 Energy-based 算法 (T={temp})...")

    # --- 1. 计算 Energy Score ---
    # Score = T * logsumexp(logits / T)
    # Score 越大 -> 能量越低 -> 越确信是 ID
    # Score 越小 -> 能量越高 -> 越可能是 OoD
    ...
    # 【AUROC】: 方向无关，假设 ID 分数高 (1)，OoD 分数低 (0)
    ...
    # 【AUPR-In】 (ID as Positive): 这是你之前 MSP (31%) 和 Energy (33%) 的计算方式
    # Baseline = P / (P+N) ≈ 288/9300 ≈ 3%
    aupr_in = average_precision_score(y_true, y_scores)

    # 【AUPR-Out】 (OoD as Positive): 这是你之前 DGP (99%) 的计算方式
    # Baseline = N / (P+N) ≈ 9000/9300 ≈ 97%
    # 注意：对于 AUPR-Out，我们需要反转分数（因为 Energy 分数越小越是 OoD）
    ...
    # 取第 5% 分位点 (例如 100个数里第5小的数)
    threshold_95 = sorted_train[int(len(sorted_train) * 0.05)]
    
    # FPR: 有多少 OoD 样本的分数竟然比阈值还大 (被误判为 ID)
