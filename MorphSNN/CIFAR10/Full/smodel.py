...
class Model(nn.Module):
    def __init__(self, node_num=5, in_channels=2, out_channels=64, num_classes=11, M=2, k_pruning=3, trace_decay=0.6,
                 beta_plasticity=0.2):
        ...
        self.stsp_block1 = STSPBlock(node_num, out_channels, out_channels,
                                     k_pruning=self.k_pruning, beta_plasticity=self.beta_plasticity,
                                     trace_decay=self.trace_decay, M=self.M)

        self.stsp_block2 = STSPBlock(node_num, out_channels, out_channels * 2,
                                     k_pruning=self.k_pruning, beta_plasticity=self.beta_plasticity,
                                     trace_decay=self.trace_decay, M=self.M)

        self.stsp_block3 = STSPBlock(node_num, out_channels * 2, out_channels * 4,
                                     k_pruning=self.k_pruning, beta_plasticity=self.beta_plasticity,
                                     trace_decay=self.trace_decay, M=self.M)

        self.stsp_block4 = STSPBlock(node_num, out_channels * 4, out_channels * 8,
                                     k_pruning=self.k_pruning, beta_plasticity=self.beta_plasticity,
                                     trace_decay=self.trace_decay, M=self.M)
        ...
        self.fc1 = nn.Linear(out_channels * 8, out_channels)
    ...
    def forward(self, x, return_graph=False):
        ...
        out = self.stsp_block1(out)

        out = self.stsp_block2(out)

        out = self.stsp_block3(out)

        out = self.stsp_block4(out)
...
if __name__ == '__main__':
    ...
    net = Model(node_num=5, in_channels=3, out_channels=32, num_classes=10).to(device)
    ...
    # x = torch.round(torch.rand((16, 1, 3, 32, 32))).to(device="cuda")
