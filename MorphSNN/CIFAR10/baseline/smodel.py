...
class RandWire(nn.Module):
    def __init__(self, node_num, p, in_channels, out_channels, graph_mode, output_dir):
        ...
        self.nodes = [0, 1, 2, 3]
        self.in_edges = {0: [], 1: [0], 2: [0, 1], 3: [0, 1, 2]}
...
class Model(nn.Module):
    def __init__(self, node_num, p, in_channels, out_channels, graph_mode, output_dir):
        ...
        self.randwire1 = RandWire(self.node_num, self.p, self.in_channels, self.out_channels * 2, self.graph_mode,
                                  self.output_dir)

        self.randwire2 = RandWire(self.node_num, self.p, self.in_channels * 2, self.out_channels * 4, self.graph_mode,
                                  self.output_dir)

        self.randwire3 = RandWire(self.node_num, self.p, self.in_channels * 4, self.out_channels * 8, self.graph_mode,
                                  self.output_dir)
        ...
        self.CIFAR_classifier = nn.Linear(self.out_channels * 8, self.num_classes, bias=True)
...
if __name__ == "__main__":
    x2 = torch.round(torch.rand((4, 1, 2, 32, 32))).to("cuda")
    net = Model(2, 0.75, 64, 64, 'WS', './test').to(device="cuda")
    # print(net)
    print(net(x2).shape)
