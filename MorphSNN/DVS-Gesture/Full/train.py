...
def main():
    ...
    import smodel
    net = smodel.Model(args.N, 2, 32, M=args.M, k_pruning=args.K, trace_decay=args.trace, beta_plasticity=args.beta).to(
        device)
    # import baseline

    # net = baseline.Model(5, 0.75, 32, 32, 'WS', './test').to(device="cuda")
    size = 128
    # print(net)
