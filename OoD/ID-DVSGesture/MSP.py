...
def get_logits(model, loader, device, T=5):
    ...
    with torch.no_grad():
        for x, y in tqdm(loader, desc="Extracting logits"):
            ...
            functional.reset_net(model)  # Important: Reset SNN state

    return np.concatenate(all_logits)


def run_msp_analysis(logits_train, logits_test, logits_ood):
    print("\n>>> Starting MSP algorithm...")

    # 1. Softmax Winners (Confidence Score)
    # MSP: Higher score -> More confident as ID
    ...
    # (1) AUROC
    # ID=1, OoD=0. Higher scores for ID
    ...
    # Note: MSP scores are lower for OoD, so take negative
    ...
    # False positive: OoD samples with scores > threshold (misclassified as ID)
...
if __name__ == "__main__":
    ...
    # Step 1: Extract Logits (this is the input for MSP class)
