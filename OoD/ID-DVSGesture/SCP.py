...
# ==========================================
# 2. Core: Feature extraction (fix dimension issues)
# ==========================================
def get_spikes_and_preds(model, loader, device):
    ...
    with torch.no_grad():
        for x, y in tqdm(loader, desc="Extracting"):
            ...
            # Key fix: DVS data is typically [B, T, C, H, W]
            # SNN needs [T, B, C, H, W]
            x = x.transpose(0, 1)
...
# ==========================================
# 3. SCP Algorithm part
# ==========================================
def compute_archetypes(train_spikes, train_labels, num_classes):
    archetypes = {}
    print(f"\n>>> Phase 1: Building SCP representations (Total Classes: {num_classes})...")

    for c in range(num_classes):
        class_data = train_spikes[train_labels == c]

        # If a class has no data (which may happen when DVS dataset is unevenly split), skip or raise error
        if len(class_data) == 0:
        ...
        try:
            ...
            archetypes[c] = np.array([np.median(class_data, axis=0)])  # Use global median as fallback if clustering fails
...
def compute_scp_scores(spikes, pred_labels, archetypes):
    ...
    for i in tqdm(range(len(spikes)), desc="Scoring"):
        ...
        if pred not in archetypes or len(archetypes[pred]) == 0:
            scores.append(1e9)  # Abnormal case
            continue
...
def run_scp_analysis(scores_train, scores_test, scores_ood):
    print("\n>>> Starting SCP performance evaluation...")
...
if __name__ == "__main__":
    ...
    # 1. Model loading
    # Gesture has 11 classes, 2 input channels
    ...
    # 3. Data loading (strictly according to your requirements)
    ...
    # Random sampler. Can just return a random order of batch data
