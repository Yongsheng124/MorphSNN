...
class SymmetricGATbuilder(nn.Module):
    """
    Symmetric graph attention layer for constructing dynamic adjacency matrices.
    Different from standard GAT:
...
class STSPBlock(nn.Module):
    ...
    def forward(self, x, return_graph=False):
        ...
        for t in range(T):
            ...
            # Here call the new function, convert directed S_pruned to undirected normalized P
            # This step corresponds to: Minimizing Dirichlet Energy in theory
            ...
            # 4. Source injection and diffusion fusion (Source Injection & Fusion)
            # Formula: H_out = (1 - gamma) * X_source + gamma * H_diffused
            # Physical meaning: Ensure source node information is retained while allowing information to diffuse throughout the network
...
# ==========================================
# 3. Overall Model (DVS Gesture Adaptation)
# ==========================================

class Model(nn.Module):
    def __init__(self, node_num=5, in_channels=2, out_channels=64, num_classes=11, M = 2, k_pruning = 3, trace_decay = 0.6, beta_plasticity = 0.2):
        ...
        self.stsp_block = STSPBlock(node_num, out_channels, out_channels,
                                    k_pruning=self.k_pruning, beta_plasticity=self.beta_plasticity, trace_decay=self.trace_decay, M= self.M)
    ...
    def forward(self, x, return_graph=False, return_spike = False):
        ...
        spike_counts = out.sum(0)
        out_mean = out.mean(0)
      
        if return_graph:
            return self.fc(out_mean), graph_tensor

        if return_spike:
            return self.fc(out_mean), spike_counts
...
if __name__ == '__main__':
    ...
    net = Model(node_num=5, in_channels=2, out_channels=32, num_classes=11, M = 2).to(device)
