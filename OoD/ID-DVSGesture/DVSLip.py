...
class DVSLipDataset(Dataset):
    def __init__(self, data_root, label_dct, train=True, augment_spatial=False, augment_temporal=False, T=90, Tnbmask=6,
                 Tmaxmasklength=18):
        self.filenames = []
        self.labels = []
        ...
        for root, dirs, files in os.walk(data_root):
            for filename in files:
                # ✅ Use os.path.basename to automatically handle separators for different systems
                word = os.path.basename(root)

                # If sometimes root ends with separator, may need to strip first, or use split(os.sep)
                if not word:  # Handle possible empty string case
                    word = root.split(os.sep)[-1]
    ...
    def __getitem__(self, idx):
        ...
        # Add dimension adjustment to 128×128
        import torchvision.transforms as trans
        final_transform = trans.Compose([trans.Resize(128)])
        item = final_transform(item)  # (T, Cin, 88, 88) → (T, Cin, 128, 128)
