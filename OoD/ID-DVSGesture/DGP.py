...
# ==========================================
# 1. Core: Extract Graph Structure Features (Graph Structure S)
# ==========================================
def get_graph_features(model, loader, device):
    """
    Extract dynamic graph structure S for each sample.
    Returns:
        features: Flattened vectors of [Samples, T * N * N]
        labels: [Samples]
    """
    model.eval()
    all_graphs = []
    ...
    print(f"Extracting Graph Structure Features S (samples: {len(loader.dataset)})...")
    ...
    with torch.no_grad():
        for x, y in tqdm(loader, desc="Extracting Graphs"):
            ...
            # Return value: (logits, graph_tensor)
            # graph_tensor shape: [B, T, N, N]
            _, graph_tensor = model(x, return_graph=True)

            # Flatten graph structure as feature vector
            # [B, T, N, N] -> [B, T * N * N]
...
def compute_graph_prototypes(train_graphs, train_labels, num_classes=11):
    """
    Compute graph prototype centers for each class
    """
    prototypes = {}
    print("\nComputing Graph Structure Prototypes...")
    
    for c in range(num_classes):
        # Get all graph vectors for this class
        ...
        # Calculate mean as the "standard graph pattern" for this class
        # Can also try Median or K-Means clustering
...
def calculate_ood_scores(features, prototypes):
    """
    Calculate distance from each sample to nearest graph prototype
    Larger distance -> Less conform to known graph patterns -> OoD
    """
    scores = []
    num_classes = len(prototypes)
    
    # Stack all prototypes into matrix [Num_Classes, Feat_Dim]
    proto_matrix = np.vstack([prototypes[c] for c in range(num_classes)])
    
    # Batch calculate distances (using L2 distance, can also try 'cityblock' L1)
    # dists: [N_samples, Num_Classes]
    dists = cdist(features, proto_matrix, metric='euclidean')
    
    # Take distance to nearest class as score
...
if __name__ == "__main__":
    ...
    # 5. SCP process (based on Graph Structure)
    # Calculate graph prototypes for ID training set
    # Note: Gesture has 11 classes (0-10)
    ...
    # Plot histogram of graph structures
