...
# from spikingjelly.clock_driven import layer # 这一行如果没用到 SNN 组件可以注释掉，或者保留
# from spikingjelly.clock_driven.neuron import MultiStepParametricLIFNode # 同上

# 简单的 ANN 组件模拟 (如果你没有 spikingjelly 环境，可以用这个占位，有的话保留上面 import)
# 如果有 spikingjelly，请取消下面这行的注释并注释掉 class layer
...
class STSPBlock(nn.Module):
    ...
    def compute_diffusion_operator(self, adj):
        ...
        # I = torch.eye(N, device=adj.device).unsqueeze(0).expand(B, N, N) # 自环可选
        adj_hat = adj_sym
        degree = adj_hat.sum(dim=2).clamp(min=1e-6)  # 防止除以0
    ...
    def forward(self, x):
        ...
        # 【修复】更安全的展平方式，避免 squeeze(0) 的潜在风险
...
class ResidualWrapper(nn.Module):
    def __init__(self, node_num, in_channels, out_channels):
        ...
        # STSPBlock 总是会下采样 (Pool Node 0)，所以 Shortcut 必须匹配
